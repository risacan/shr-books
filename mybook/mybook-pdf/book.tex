...
\documentclass[dvipdfmx,uplatex,papersize,twoside,a5j,9pt,openany]{jsbook}
% %% fixes to LaTeX2e
...
\def\starter@draft{}
\def\starter@chapter@decoration{underline}
...
\def\starter@program@ttfont{courier}
\def\starter@terminal@ttfont{courier}
\def\starter@program@widen{0.0mm}
\def\starter@terminal@widen{0.0mm}
\def\starter@colophon@poweredby{}
...
\usepackage[dvipdfmx,bookmarks=true,bookmarksnumbered=true,colorlinks=true,%
     pdftitle={はじめてのXXX入門},%
     pdfauthor={カウプラン機関極東支部}]{hyperref}
...
{\noindent\reviewtitlefont\Large はじめてのXXX入門} \\
\medskip
{\noindent\reviewtitlefont 概念と基本操作を2時間でマスター！} \\
